...
```
todo,task,planner,productivity,tasks,to-do,anxiety,procrastination,mental health,calm,gentle,mindful
```
...
```
为难熬的日子准备的安静待办应用。没有连胜，没有定额，没有评分。只是温柔的进展，以及为你记录"是什么在阻碍你"的备注。
```

**Description**
```
Sagely 是一款安静的待办应用，专为那些发现传统效率工具——带着连胜、计分板、"你错过了目标"的通知——实际上让自己的焦虑、强迫倾向或拖延变得更糟的人设计。

没有连胜。没有定额。没有"连续 X 天"。如果今天很难，那今天就是难。Sagely 不会因此给你打分。
...
• 自定义颜色和 80+ 图标的分类——可拖动排序
• 用 Apple、Google 或邮箱登录
• 通过安全云存储跨设备同步;支持离线模式,任何地方都可以记录
...
Sagely 不会让你更高效。但它可能让那些高效的时刻不那么痛苦。
```

**Keywords** (100 chars max)
```
焦虑,拖延,心理健康,平静,温柔,待办,任务,提醒,计划,正念,mochi,强迫症
```
...
**Promotional text** (170 chars max)
```
Una app de tareas tranquila para los días en que no puedes ser una persona productiva. Sin rachas. Sin cuotas. Sin calificaciones. Solo progreso gentil y notas.
```

**Description**
```
Sagely es una app de tareas tranquila para personas que descubren que las herramientas de productividad convencionales — con sus rachas, marcadores y notificaciones de "no cumpliste tu meta" — empeoran su ansiedad, TOC o procrastinación.

No hay rachas. No hay cuotas. No hay "X días seguidos". Si hoy fue difícil, hoy fue difícil. Sagely no te califica por ello.
...
• Categorías con colores personalizados y una biblioteca de más de 80 íconos — arrastra para reordenar
• Inicia sesión con Apple, Google o correo
• Sincronización entre dispositivos con almacenamiento seguro en la nube; soporte sin conexión para que puedas escribir donde sea
...
Sagely no te hará más productivo. Pero podría hacer que los momentos productivos duelan menos.
```

**Keywords** (100 chars max)
```
ansiedad,procrastinación,salud mental,TOC,tranquilo,tareas,recordatorios,planificador,mindful,mochi
```
...
**Promotional text** (170 chars max)
```
Une app de tâches calme pour les jours où tu ne peux pas être quelqu'un de productif. Pas de série. Pas de quota. Pas de notation. Juste du progrès doux et des notes.
```

**Description**
```
Sagely est une app de tâches calme pour les personnes qui trouvent que les outils de productivité conventionnels — avec leurs séries, leurs tableaux de score et leurs notifications "tu as manqué ton objectif" — aggravent leur anxiété, leur TOC ou leur procrastination.

Pas de série. Pas de quota. Pas de "X jours d'affilée". Si la journée a été dure, elle a été dure. Sagely ne te note pas pour cela.
...
• Catégories avec couleurs personnalisées et plus de 80 icônes — réorganise par glissement
• Connexion avec Apple, Google ou email
• Synchronisation entre appareils via stockage cloud sécurisé ; mode hors ligne pour écrire n'importe où
...
Sagely ne te rendra pas plus productif. Mais elle pourrait rendre les moments productifs un peu moins douloureux.
```

**Keywords** (100 chars max)
```
anxiété,procrastination,santé mentale,TOC,calme,tâches,rappels,planificateur,doux,mochi
```
...
**Promotional text** (170 chars max)
```
Eine ruhige To-Do-App für Tage, an denen du keine produktive Person sein kannst. Keine Streaks. Keine Quoten. Keine Bewertung. Nur sanfter Fortschritt und Notizen.
```

**Description**
```
Sagely ist eine ruhige To-Do-App für Menschen, die feststellen, dass herkömmliche Produktivitäts-Tools — mit ihren Streaks, Punktetafeln und "Du hast dein Ziel verfehlt"-Benachrichtigungen — ihre Angst, ihren Zwang oder ihre Aufschieberitis aktiv verschlimmern.

Keine Streaks. Keine Quoten. Kein "X Tage in Folge". Wenn heute schwer war, war heute schwer. Sagely bewertet dich nicht dafür.
...
• Kategorien mit eigenen Farben und einer Bibliothek von über 80 Symbolen — zum Umsortieren ziehen
• Anmeldung mit Apple, Google oder E-Mail
• Synchronisierung zwischen Geräten über sicheren Cloud-Speicher; Offline-Modus zum Schreiben überall
...
Sagely macht dich nicht produktiver. Aber sie könnte die produktiven Momente etwas weniger weh tun lassen.
```

**Keywords** (100 chars max)
```
Angst,Aufschieberitis,psychische Gesundheit,Zwang,ruhig,Aufgaben,Erinnerung,Planer,achtsam,mochi
```
...
**Promotional text** (170 chars max)
```
生産的な人になれない日のための、静かなToDoアプリ。連続記録なし、ノルマなし、評価なし。ただ穏やかな進歩と、「何があなたを止めているか」のメモだけ。
```

**Description**
```
Sagely は、従来の生産性ツール—連続記録、スコアボード、「目標を逃しました」という通知—が、自分の不安、強迫傾向、先延ばし癖を実際に悪化させていると気づいた人のための、静かなToDoアプリです。

連続記録はありません。ノルマもありません。「○日連続」もありません。今日が辛かったなら、今日は辛かった。Sagely はそれで評価しません。
...
• カスタムカラーと80以上のアイコンライブラリのカテゴリー—ドラッグで並び替え
• Apple、Google、メールでサインイン
• 安全なクラウドストレージでデバイス間同期；オフラインでも書き込み可能
...
Sagely はあなたをより生産的にはしません。でも、生産的な瞬間が少し痛みにくくなるかもしれません。
```

**Keywords** (100 chars max)
```
不安,先延ばし,メンタルヘルス,OCD,落ち着いた,やさしい,ToDo,タスク,リマインダー,プランナー,mochi
```
